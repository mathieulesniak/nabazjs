...
app.get('/1.0/rabbit/insomniac', function (req, res) {
    var rabbit = burrow.getRabbit(req.query.rabbit);
    if (rabbit != false) {
        rabbit.insomniac = parseInt(req.query.insomniac);
        if (rabbit.isAsleep()) {
            rabbit.wakeUp();
        }
    }
...
app.get('/1.0/rabbit/save-config', function (req, res) {
    var rabbit = burrow.getRabbit(req.query.rabbit);
    if (rabbit != false) {
        rabbit.config.friendlyName  = req.query.friendlyName;
        rabbit.config.breathe       = req.query.breathe;
        rabbit.config.taichi        = req.query.taichi;
        rabbit.config.surprise      = req.query.surprise;
        
        rabbit.saveConfig();
        rabbit.applyConfig();
        res.end("ok");
    } else {
        res.end("unknown rabbit");
    }
...
app.get('/1.0/rabbit/surprise', function (req, res) {
    burrow.getRabbit(req.query.rabbit).launchPlugin('surprise');
    res.end("ok");
});
    ...
    if (typeof req.query.mp3 == 'undefined') {
        var exec = require('child_process').exec,
            ...
            child = exec('/usr/bin/wget -q -U Mozilla "http://translate.google.com/translate_tts?ie=UTF-8&tl=fr&q=' + encodeURIComponent(req.query.text) + '" -O ./storage/tts/' + rabbit.macAddress + '.mp3', function(error, stdout, stderr) {
                ...
                var files           = plugin.getMp3FromDirectory(respirationDir);
                var respirationNb   = Math.floor(Math.random() * (files.length) + 1);

                var cmd = 'MU http://' + serverConfig.httpHost + respirationDir + '/' + respirationNb + '.mp3' + "\n"
                    + 'MW' + "\n"
                    + 'MU http://' + serverConfig.httpHost + '/1.0/rabbit/speak?rabbit=' + rabbit.macAddress + '&mp3=1' + "\n"
                    + 'MW' + "\n"
