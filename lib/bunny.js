...
bunny.prototype =  extend(EventEmitter.prototype, {
    ...
    export: function() {
        return {'jid': this.getJid(), 'macAddress': this.macAddress, 'nickname': this.nickname, 'ip':this.getIp(), 'status': this.getStatus()};
    },
    loadConfig: function() {
        var that = this;
    ...
    handleMessageMessage: function(parsedMessage) {
        if (typeof parsedMessage.message.button != 'undefined') {
            ...
            http.get("http://192.168.100.1:2222/1.0/rabbit/speak?rabbit=" + this.macAddress + "&text=Touche pas à mes oreilles");
        }
    },
    talk: function(text) {
